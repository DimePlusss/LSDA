...
class DfToStringMapping:
    ...
    def add(self, name: str, ti: TableInfo) -> None:
        ...
        """
        #print("yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy ich will wissen ob in dftostring gemappt wird?")
...
class ViewMapping:
    ...
    #Changed
    def add(self, name: str, dfI: DfInfo) -> None:
        ...
        if dfI.helper_source_code.count('(')==dfI.helper_source_code.count(')'):
            self.mapping = [(name, dfI), *self.mapping]  # Quite efficient way to add values at the front.
        else: 
            print("name ", name, dfI)
    ...
    #changed
    def get_name_and_ti(self, df_to_find, helper_source_code) -> str:
        ...
        for x in self.mapping:
            if x[1].helper_source_code==helper_source_code:
                x[1].dfName=""
            if x[1].dfName == df_to_find:
                name_and_view=x
                break
            
        return name_and_view
        #return next(x for x in self.mapping if x[0].data_object is df_to_find)
