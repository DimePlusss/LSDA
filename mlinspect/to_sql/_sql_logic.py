...
class SQLLogic:
    ...
    def __init__(self, viewmapping, mapping, pipeline_container, dbms_connector, sql_obj, id=1):
    ...
    def handle_operation_series(self, operator, result, targetdf, sourcedf, helper_source_code, left, right, line_id):
        """
        ...
        #part for method match
        elif isinstance(left, pandas.core.strings.accessor.StringMethods):
            tb1=left._orig 
            name_l, ti_l = self.mapping.get_name_and_ti(tb1)      
            
            if isinstance(right, str):
                if "*" in right :
                    right = f"%" +right.split("*")[1]
                if "$" in right:
                    right= right.split("$")[0]
                else:
                    right= right + f"%"
        ...
        cte_name, sql_code = self.finish_sql_call(sql_code, targetdf, helper_source_code, line_id, result,
    ...
    def finish_sql_call(self, sql_code, targetdf, helper_source_code, line_id, result, tracking_cols, operation_type, non_tracking_cols_addition=[],
