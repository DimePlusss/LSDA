...
#XXX to print the ast
from pprint import pprint
#from ._call_capture_transformer import iterate_Assign_nodes
...
class PipelineExecutor:
    ...
    def run(self, *,
        ...
        if self.to_sql:
            ...
            self.sql_logic = SQLLogic(viewmapping=self.viewmapping, mapping=self.mapping, pipeline_container=self.pipeline_container,
            ...
            self.backup_eq_string = pandas.Series.str.match
    ...
    def run_inspections(self, notebook_path, python_code, python_path):
        ...
        #print(ast.dump(parsed_modified_ast))
        
        lol, bol, lastop= iterate_Assign_nodes(parsed_ast)
...
def iterate_Assign_nodes(node, annotate_fields=True, include_attributes=False, *, indent=None):
    """ 
    Code from ast.dump reused and slightly modified
    """
    def _format(node, last_operation, level=0):
        ...
        if isinstance(node, ast.AST):
            if isinstance(node, ast.Assign):
                z=node.value
                for znode in ast.walk(node):
                    if isinstance(znode, ast.Attribute):
                        if(znode.attr in ["groupby", "head", "sort_values", "agg","merge","read_sql_query","match","reset_index","isin",]):
                            singleton.last_operation=node
                    if isinstance(znode, ast.Subscript):
                        singleton.last_operation=node
            ...
            for name in node._fields:
                ...
                if value is None and getattr(cls, name, ...) is None:
                    keywords = True
                    continue
                value, last_operation, simple = _format(value, last_operation, level)
            ...
            if allsimple and len(args) <= 3:
                return '%s(%s)' % (node.__class__.__name__, ', '.join(args)), not args, last_operation
            return '%s(%s%s)' % (node.__class__.__name__, prefix, sep.join(args)), False, last_operation
        elif isinstance(node, list):
            if not node:
                return '[]', True, last_operation
            return '[%s%s]' % (prefix, sep.join(_format(x, last_operation, level)[0] for x in node)), False, last_operation
        return repr(node), True, last_operation
    ...
    return _format(node, last_operation=None)
