...
#from mlinspect.instrumentation._pipeline_executor import get_monkey_patching_patch_sources
#import gorilla
...
def execute_patched_func(original_func, execute_inspections_func, *args, **kwargs):
    ...
    caller_filename = sys._getframe(
        2).f_code.co_filename  # pylint: disable=protected-access
    ...
    if caller_filename != singleton.source_code_path:
        ...
        # Dieser part wird erstellt um die variable vor dem code also zb df = ... herauszufinden, damit wir diese speichern können mit ihrem dazugehörigem SQL-VIEW namen und abrufen können, wenn diese variable/dataframe verwendet wird
        call_helper_ast_node = ast.Call(lineno=singleton.lineno_next_call_or_subscript,
                                 col_offset=0,
                                 end_lineno=singleton.end_lineno_next_call_or_subscript,
                                 end_col_offset=singleton.end_col_offset_next_call_or_subscript)

        helper_source_code = ast.get_source_segment(
            singleton.source_code, node=call_helper_ast_node)
        targetdf=""
        if len(helper_source_code.split("="))>1:
            targetdf=helper_source_code.split("=")[0]
            if len(targetdf.split("["))>1:
                targetdf= targetdf.split("[")[0]

        sourcedf=""
        if len(caller_source_code.split("["))>1:
            sourcedf=caller_source_code.split("[")[0]
            if len(sourcedf.split("."))>1:
                sourcedf=sourcedf.split(".")[0]
            elif len(sourcedf.split("("))>1:
                sourcedf=sourcedf.split("(")[1]
            sourcedf= "".join([s for s in sourcedf.strip().splitlines(True) if s.strip("\r\n")])  
             
        # Code to find the last patched operation
        last_op_code =ast.get_source_segment(singleton.source_code, singleton.last_operation)
        

        last_op = (helper_source_code==last_op_code)    
        if "__op_call_helper" in str(execute_inspections_func):
            last_op=False
        ...
        result=execute_inspections_func(op_id, caller_filename, caller_lineno, caller_code_reference,
                                          caller_source_code, targetdf, sourcedf, helper_source_code, last_op)
    else:
        op_id=singleton.get_next_op_id()
        caller_lineno=sys._getframe(
            2).f_lineno  # pylint: disable=protected-access
...
def execute_patched_func_no_op_id(original_func, execute_inspections_func, *args, **kwargs):
    ...
    if caller_filename != singleton.source_code_path:
        ...
        if len(helper_source_code.split("="))>1:
            targetdf=helper_source_code.split("=")[0]
        # Hier endet der part zum testen von den ergebnis dataframe name speichern        
        # Hier beginnt der part um sourcedf zu erkennen
        sourcedf=""
        if len(caller_source_code.split("["))>1:
            sourcedf=caller_source_code.split("[")[0]
            if len(sourcedf.split("."))>1:
                ...
                sourcedf=sourcedf.split("(")[1]
            sourcedf= "".join([c for c in sourcedf.strip().splitlines(True) if c.strip("\r\n")])
            #sourcedf = filter(lambda x: not re.match(r'^\s*$', x), sourcedf)
        
        # Hier endet der part mit der sourcedf        
        ...
        result=execute_inspections_func(-1, caller_filename, caller_lineno, caller_code_reference,
                                          caller_source_code, targetdf, sourcedf, helper_source_code, False)
        
    else:
        caller_lineno=sys._getframe(
            2).f_lineno  # pylint: disable=protected-access
        result=execute_inspections_func(-1,
                                        caller_filename, caller_lineno, None, None)
    return result
...
def get_input_info(df_object, caller_filename, lineno, function_info, optional_code_reference, optional_source_code) \
    ...
    if isinstance(df_object, DataFrame):
        ...
        columns=list(df_object.columns)
        ...
        raise NotImplementedError(
            "TODO: Mlinspect info storage for type: '{}'".format(type(df_object)))
...
def add_dag_node(dag_node: DagNode, dag_node_parents: List[DagNode], backend_result: BackendResult):
    ...
    if annotated_df is not None:
        singleton.inspection_results.dag_node_to_inspection_results[
            dag_node]=backend_result.dag_node_annotation
